...
module.exports.run = async function({ api, event, args }) {
  ...
  if (!input) {
    return api.sendMessage("Please provide a question or statement after 'ai'.", event.threadID, event.messageID);
  }

  if (input === "clear") {
    try {
      await axios.post('gpt-4-cfgh.onrender.com/clear', { id: event.senderID });
  ...
  try {
    ...
    const { data } = await axios.post('gpt-4-cfgh.onrender.com/chat', {
      prompt: input,

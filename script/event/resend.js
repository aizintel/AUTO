  ...
  if (event.type == "message_unsend" && msgData.hasOwnProperty(event.messageID)) {
    ...
    if (msgData[event.messageID].attachments.length === 0) {
      ...
      for (const item of msgData[event.messageID].attachments) {
        ...
        fs.writeFileSync(__dirname + `/cache/${item.filename}.jpg`, Buffer.from(data))
        photo.push(fs.createReadStream(__dirname + `/cache/${item.filename}.jpg`))
        del.push(__dirname + `/cache/${item.filename}.jpg`)
      }
      api.sendMessage({
      ...
      let {
        data
      } = await axios.get(msgData[event.messageID].attachments[0].url, {
        responseType: "arraybuffer"
      })
      fs.writeFileSync(__dirname + `/cache/audio.mp3`, Buffer.from(data))
      api.sendMessage({
        body: `${name} unsent this voice message: ${msgData[event.messageID].body}`,
        attachment: fs.createReadStream(__dirname + '/cache/audio.mp3')
      }, event.threadID, () => {
        fs.unlinkSync(__dirname + '/cache/audio.mp3')
      });
    } else if (msgData[event.messageID].attachments[0].type == 'animated_image') {
      let {
        data
      } = await axios.get(msgData[event.messageID].attachments[0].previewUrl, {
        responseType: "arraybuffer"
      })
      fs.writeFileSync(__dirname + `/cache/animated_image.gif`, Buffer.from(data))
      api.sendMessage({
        body: `${name} unsent this gif: ${msgData[event.messageID].body}`,
        attachment: fs.createReadStream(__dirname + '/cache/animated_image.gif')
      }, event.threadID, () => {
        fs.unlinkSync(__dirname + '/cache/animated_image.gif')
      });
    }
  }
}
